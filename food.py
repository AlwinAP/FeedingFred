...
class Junk(Image):
    ...
    def __init__(self, what=None, lvl=1, image="dialog-information.png", **kwargs):
        ...
        self.size = (48, 48)
        self.calories = randint(*self.storehouse[what]["calories"])
        self.speed_mod = self.storehouse[what].get("speed_mod", 0.5)
        self.bind(active=self.sinking)
        self.bind(
            parent=lambda instance, value: instance.animation.unbind(on_complete=self.sunk) if not value else False)
...
class FoodScoreFeedback(Label):
    def __init__(self, **kwargs):
        ...
        if score > 0:
            anim = Animation(y=self.y + 150, d=0.03)
        else:
            anim = Animation(y=self.y + 100, d=0.03)
        Clock.schedule_once(self.dissapear, 0.4)
        anim.start(self)
