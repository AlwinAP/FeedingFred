...
class Fish(Scatter):
    ...
    rank = ["a fry", "a cat", "a car", "a whale", "a candy store", "an oil tanker", "the Iceland",
            "the Pacific Ocean itself!", 'the "MAFIAA"']
    ...
    def __init__(self, image="images/fish.png", box=[0, 0, 100, 100], **kwargs):
        ...
        self.bind(active=lambda instance, value: Clock.schedule_interval(instance.consume_calories,
                                                                         0.5) if value else Clock.unschedule(
            instance.consume_calories))
        # Dynamic entry
        self.bind(active=lambda instance, value: Animation(y=Window.height - 400, t="out_back", d=1.2).start(
            instance) if value else True)
        # Too many calories makes fish big
    ...
    def lvlup(self, instance, value):
        try:
            if self.total_calories >= self.lvlup_on_calories[self.obese_lvl]:
                ...
                self.image.size = (self.image.width * self.size_increment[self.obese_lvl - 1],
                                   self.image.height * self.size_increment[self.obese_lvl - 1])
    ...
    def on_touch_up(self, touch):
        ...
        anim = Animation(
            center=(self.target_pos[0] + sin(radians(angle)) * speed, self.target_pos[1] - cos(radians(angle)) * speed),
            t="out_cubic", d=0.6)
        anim.start(self)
